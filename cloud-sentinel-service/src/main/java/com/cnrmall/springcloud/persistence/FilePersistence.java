package com.cnrmall.springcloud.persistence;
 ...
 *
 * sentinel 规则持久化到 文件中
 * 1. 首先Sentinel控制台通过API将规则推送至客户端并更新到内存中，接着注册的写数据源会将新的规则保存到本地的文件中。
 * 2. 在resource下创建目录META-INF/services，然后添加文件
 *      com.alibaba.csp.sentinel.init.InitFunc在文件中添加配置类的全路径\
 *
 * 开启文件持久化，Nacos 持久化失效了。
 */
@Component
public class FilePersistence implements InitFunc {

    @Value("${spring.application.name:service}")
    private String applicationName = "sentinel-service";
